...
A = modis_read.read_modis_original('lists_data/list_A_0.txt', 'lists_data/list_A_1.txt', 'lists_data/list_A_2.txt')
...
train_set = Crop_Generator(A, validation = False, target ="CDL")
...
for epoch in range(1, num_epoch + 1):
    ...
    # Training loop
    model.train()
    for i, [inputs, cdl] in enumerate(train_gen):
        ...
        output = output*mask
        target = cdl*mask #so they both have the same values in the pad
...
#save model
print("Saving model...")
PATH = "nice_models_results/CDL_spatial.pth"
...
plt.savefig('nice_models_results/loss_CDL_spatial.png')

plt.plot(list(range(len(lrs))), lrs)
plt.xlabel("Batches")
plt.ylabel("Learning rate")
plt.title("Evolution of Learning Rate")
plt.savefig('nice_models_results/lr_CDL_spatial.png')

with open("nice_models_results/list_loss_CDL_spatial.txt", 'w') as f:
    for s in mse_list:
        f.write(str(s) + '\n')
